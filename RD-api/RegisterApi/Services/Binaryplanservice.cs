...
public class BinaryPlanService : IBinaryPlanService
{
    ...
    public async Task<(bool success, string message, BinaryWithdrawalRequest? request)> RequestWithdrawalAsync(string userId, decimal amount)
    {
        ...
        // Save now so `request.Id` is generated by the DB — we need it to tag
        // the reserve transaction below, so ProcessWithdrawalAsync can find
        // and update this exact row later instead of leaving it stuck saying
        // "pending" forever.
        await _db.SaveChangesAsync();
        ...
        int requestId, bool approve, string adminNote)
    {
        ...
        // Find the original "Reserved pending withdrawal approval" row that
        // was created at request time, so we can flip its text to the final
        // status instead of leaving it stuck saying "pending" forever.
        var reserveTxn = await _db.BinaryWalletTransactions
            .Where(t => t.UserId == request.UserId
                     && t.ReferenceId == $"WR-{requestId}"
        ...
        if (approve)
        {
            ...
            {
                // Fallback in case the original row couldn't be found (e.g.
                // older request created before this fix) — still log it.
        ...
        else
        {
            ...
            if (reserveTxn != null)
            {
                reserveTxn.Source = "Withdrawal Rejected";
                reserveTxn.Description = $"Withdrawal rejected. Ref: WR-{requestId}";
            }
