...
public class OrdersController : ControllerBase
{
    ...
    public async Task<IActionResult> ApprovePayment(int id, [FromBody] ProcessPaymentOrderDto dto)
    {
        ...
        // ── Credit the buyer's BusinessVolume (BV) ──────────────────────────
        // PlanController.Checkout does "user.BusinessVolume += (int)totalBv;"
        // for Plan purchases, but that line was missing here — so shop orders
        // approved through this endpoint paid commission but never increased
        // the buyer's own BV total. Mirroring the same update here so both
        // purchase paths keep BusinessVolume in sync.
        ...
        {
            // Shouldn't happen (order.UserId should always reference a real user),
            // but don't silently swallow it — surface it the same way the
            // commission failure below is surfaced, since BV is on the line too.
